      ...
      <div className="lobby-settings-content">
        ...
        <div className="lobby-voice-section">
          ...
          <div className="lobby-vc-hint">
            {tl('选择一个语音频道后，你将', 'After choosing a voice channel, you will ')}
            <strong>
              {tl('只能听到同频道玩家的声音', 'only hear players in the same channel')}
            </strong>
            {tl('，也只有同频道的人能听到你的声音。', ', and only they can hear you.')}
          </div>
        ...
        <div className="lobby-config-box">
          <div className="lobby-config-section-head">
            <span className="lobby-config-section-title">
              {tl('EasyTier 网络配置', 'EasyTier Network Config')}
            </span>
            <span className="lobby-config-section-note">
              {tl(
                '仅本区域配置需要点下方「保存」，语音设备与频道选择即时生效',
                'Only this section needs Save below; audio device and channel changes apply instantly'
              )}
            </span>
          </div>
              ...
              <div className="global-config-hint">
                ...
                <span>
                  {tl(
                    ...
                    'Using global config; edit it in MCTier Settings'
                  )}
                </span>
              </div>
            )}
            ...
            {!useGlobalConfig && (
              <Form
                ...
                onValuesChange={() => {
                  ...
                  console.log(
                    '📝 [LobbySettings] 用户修改了表单字段，确保 use_global_config 为 false'
                  );
                  if (useGlobalConfig) {
              ...
              >
                ...
                <div style={{ marginTop: '16px', marginBottom: '8px' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--mc-text-3)', marginBottom: '8px' }}>
                    {tl(
                      '将本地端口转发到虚拟网络中的远程端口',
                      'Forward a local port to a remote port on the virtual network'
                    )}
                  </div>
                </div>
                        ...
                        >
                          ...
                          <div>
                            ...
                            >
                              <Input
                                placeholder={tl(
                                  '例如：10.126.126.1:5678',
                                  'e.g. 10.126.126.1:5678'
                                )}
                                size="small"
                                style={{ fontSize: '12px' }}
                              />
                ...
                <Form.Item
                  name="proxy_networks"
                  label={tl('子网代理 CIDR 列表', 'Subnet proxy CIDRs')}
                >
                ...
                <Form.Item
                  ...
                  label={tl('启用中继外部网络 KCP', 'Enable foreign-network relay KCP')}
                  valuePropName="checked"
                >
                  <Switch />
                </Form.Item>
                <Form.Item
                  ...
                  label={tl('外部网络流量限制（BPS）', 'Foreign network rate limit (BPS)')}
                >
