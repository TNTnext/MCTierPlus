...
export const ConnectionDiagnosticModal: React.FC<ConnectionDiagnosticModalProps> = ({
  ...
  const avgLatency =
    reachable.length > 0
      ? Math.round(reachable.reduce((s, r) => s + (r.latency || 0), 0) / reachable.length)
      : null;
  ...
  if (rows.length === 0) {
    ...
    if (relayCount > 0)
      suggestions.push(
        tl(
          `${relayCount} 位成员走中继(非直连)，延迟更高。可在高级设置尝试开启「KCP 代理 / QUIC 代理」，或关闭「禁用 P2P / 禁用 UDP 打洞」以促成直连。`,
          `${relayCount} member(s) are relayed (not P2P). Try enabling KCP/QUIC proxy or disabling "Disable P2P / Disable UDP hole punching" in advanced settings to get direct connections.`
        )
      );
    if (avgLatency != null && avgLatency > 150)
      suggestions.push(
        tl(
          '平均延迟偏高。可尝试更换更近的 EasyTier 节点，或开启「延迟优先」。',
          'Average latency is high. Try a closer EasyTier node or enable "Latency first".'
        )
      );
    if (maxLoss >= 10)
      suggestions.push(
        tl(
          '存在明显丢包，建议检查本地网络/WiFi 信号，或改用有线网络。',
          'Noticeable packet loss. Check your local network/Wi-Fi or use a wired connection.'
        )
      );
    if (offlineCount > 0)
      suggestions.push(
        tl(
          `${offlineCount} 位成员暂时不可达，可能对方未就绪或网络受限。`,
          `${offlineCount} member(s) are unreachable; they may not be ready or are network-restricted.`
        )
      );
    if (suggestions.length === 0)
      suggestions.push(
        tl('连接质量良好，无需调整。', 'Connection quality is good. No action needed.')
      );
  }

  const scoreColor = score >= 80 ? '#07c160' : score >= 50 ? '#fa9d3b' : '#fa5151';
  ...
  return (
    <Modal
      ...
      width={560}
      footer={[
        <Button key="rescan" onClick={() => void run()} loading={loading}>
          {tl('重新诊断', 'Re-run')}
        </Button>,
    ...
    >
      {loading && rows.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '32px 0' }}>
          <Spin tip={tl('正在诊断连接质量…', 'Diagnosing...')} />
        </div>
      ) : rows.length === 0 ? (
        ...
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
            <Progress
              ...
              format={(p) => <span style={{ color: scoreColor, fontWeight: 700 }}>{p}</span>}
            />
            <div style={{ fontSize: 13, color: 'var(--mc-text-2)', lineHeight: 1.9 }}>
              <div>
                {tl('成员', 'Members')}：{rows.length}
                {'\u3000'}
                {tl('直连', 'P2P')}：{rows.length - relayCount - offlineCount}
                {'\u3000'}
                {tl('中继', 'Relay')}：{relayCount}
              </div>
              <div>
                {tl('平均延迟', 'Avg latency')}：{avgLatency != null ? `${avgLatency}ms` : '—'}
                {'\u3000'}
                {tl('最高丢包', 'Max loss')}：{maxLoss}%
              </div>
              <div>
                {tl('不可达', 'Offline')}：{offlineCount}
              </div>
            </div>
          </div>
          ...
          >
            <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 6, color: '#7ccf00' }}>
              {tl('优化建议', 'Suggestions')}
            </div>
            {suggestions.map((s, i) => (
              <div key={i} style={{ fontSize: 12, color: 'var(--mc-text-2)', lineHeight: 1.7 }}>
                · {s}
              </div>
            ))}
          </div>
