...
const PRESETS: GamePreset[] = [
  {
    id: 'mc',
    name: 'Minecraft Java',
    enName: 'Minecraft Java',
    port: 25565,
    note: '建议用「局域网世界」自动发现免输IP',
    enNote: 'Tip: use LAN Worlds for auto-discovery',
  },
  {
    id: 'mcbe',
    name: '我的世界 基岩版',
    enName: 'Minecraft Bedrock',
    port: 19132,
    note: '在「好友 → 添加服务器」中填写',
    enNote: 'Add under Friends → Add Server',
  },
  {
    id: 'terraria',
    name: '泰拉瑞亚',
    ...
    port: 7777,
    note: '多人游戏 → 通过IP加入',
    enNote: 'Multiplayer → Join via IP',
  },
  {
    id: 'dst',
    name: '饥荒联机版',
    ...
    port: 10999,
    note: '需房主开服',
    enNote: 'Host must start a server',
  },
  {
    id: 'valheim',
    name: 'Valheim 英灵神殿',
  ...
  },
  {
    ...
    enName: 'CS / Source',
    port: 27015,
    note: '控制台 connect IP:端口',
    enNote: 'Console: connect IP:port',
  },
  {
    id: 'factorio',
    name: '异星工厂',
    ...
    port: 34197,
    note: '多人游戏 → 连接到地址',
    enNote: 'Multiplayer → Connect to address',
  },
];

export const GameQuickConnectModal: React.FC<GameQuickConnectModalProps> = ({
  visible,
  onClose,
}) => {
  useTranslation();
  ...
  return (
    ...
    >
      <div style={{ marginBottom: 10, fontSize: 12, color: 'var(--mc-text-3)' }}>
        {tl(
          '选择游戏后，把对应地址粘贴进游戏的「直接连接 / 输入IP」即可与好友联机。',
          'Pick a game, then paste the address into the game\'s "Direct Connect / Enter IP" to play with friends.'
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
        ...
        {selected === 'custom' && (
          <InputNumber
            ...
            max={65535}
            value={customPort}
            onChange={(v) => setCustomPort(v && v >= 1 && v <= 65535 ? v : 25565)}
            style={{ width: 110 }}
          />
        )}
      </div>
      ...
      >
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 12, color: 'var(--mc-text-3)' }}>
            {config.playerName || tl('我', 'Me')}
            {tl('（我）', ' (Me)')}
          </div>
          <div style={{ fontWeight: 600 }}>
            {selfIp ? fmt(selfIp) : tl('未分配虚拟IP', 'No virtual IP yet')}
          </div>
        </div>
        <Button type="primary" disabled={!selfIp} onClick={() => void copy(fmt(selfIp))}>
          {tl('复制地址', 'Copy')}
        </Button>
      </div>

      <div style={{ marginBottom: 8, fontWeight: 600, fontSize: 13 }}>
        {tl('加入其他玩家', 'Join others')}
      </div>
      {others.length === 0 ? (
        <div style={{ fontSize: 12, color: 'var(--mc-text-3)' }}>
          {tl('暂无其他玩家', 'No other players yet')}
        </div>
      ) : (
