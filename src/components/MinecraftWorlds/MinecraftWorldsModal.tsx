...
export const MinecraftWorldsModal: React.FC<MinecraftWorldsModalProps> = ({ visible, onClose }) => {
  ...
  const [autoLan, setAutoLan] = useState<boolean>(
    () => localStorage.getItem(MC_AUTOLAN_KEY) === '1'
  );
  ...
  const pushBroadcast = useCallback(
    async (list: DiscoveredServer[]) => {
      const payload = list
        .filter((s) => s.ip && s.ip !== selfIp)
        .map((s) => ({ ip: s.ip, port: s.port, motd: s.motd || '' }));
      try {
        await invoke('start_mc_lan_broadcast', { servers: payload });
      } catch (e) {
        console.warn('LAN 广播失败', e);
      }
    },
    [selfIp]
  );
  ...
  const handleCopy = async (server: DiscoveredServer) => {
    ...
    try {
      ...
      message.success(
        `${tl('已复制服务器地址：', 'Server address copied: ')}${addr}${tl('，在 Minecraft「多人游戏 → 直接连接」粘贴即可加入', ' — paste it in Minecraft "Multiplayer → Direct Connect" to join')}`
      );
    } catch {
      message.error(
        tl('复制失败，请手动输入地址：', 'Copy failed, please enter the address manually: ') + addr
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: 13, color: 'var(--mc-text)' }}>{tl('扫描端口', 'Scan Port')}</span>
        <Tooltip
          title={tl(
            'Minecraft「对局域网开放」默认端口为 25565；若你自建服务器或修改过端口，请填写实际端口后重新扫描',
            'The default port for Minecraft "Open to LAN" is 25565. If you run your own server or changed the port, enter the actual port and rescan'
          )}
        >
          <InputNumber
            min={1}
            ...
            onChange={(v) => {
              ...
              setPort(p);
              try {
                localStorage.setItem(MC_PORT_KEY, String(p));
              } catch {
                /* ignore */
              }
            }}
        ...
        <Button
          size="small"
          onClick={() => {
            setPort(DEFAULT_MC_PORT);
            try {
              localStorage.setItem(MC_PORT_KEY, String(DEFAULT_MC_PORT));
            } catch {
              /* ignore */
            }
          }}
      ...
      <div style={{ marginBottom: 12, fontSize: 12, color: 'var(--mc-text-3)' }}>
        {tl(
          `自动扫描大厅成员是否在端口 ${port} 上开启了 Minecraft 世界（需对方开启「对局域网开放」或自建服务器）。`,
          `Automatically scans whether lobby members have a Minecraft world open on port ${port} (they must use "Open to LAN" or run a server).`
        )}
      </div>
      ...
      >
        <Switch
          ...
          onChange={(v) => {
            setAutoLan(v);
            try {
              localStorage.setItem(MC_AUTOLAN_KEY, v ? '1' : '0');
            } catch {
              /* ignore */
            }
          }}
        />
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--mc-text)' }}>
            {tl(
              '在 Minecraft 局域网列表自动显示（免输IP）',
              'Auto-show in Minecraft LAN list (no IP typing)'
            )}
          </div>
          <div style={{ fontSize: 11, color: 'var(--mc-text-3)', marginTop: 2 }}>
            {tl(
              '开启后，大厅内发现的世界会直接出现在你 Minecraft 的「局域网」列表，点一下即可加入；保持开启即可后台持续刷新。',
              'When on, discovered worlds appear directly in your Minecraft "LAN" tab — just click to join. Keep it on to refresh in the background.'
            )}
          </div>
        </div>
      </div>
      ...
      {scanning && servers.length === 0 ? (
        ...
        <Empty
          description={tl(
            '未发现可加入的世界。请确认对方已在游戏中点击「对局域网开放」或已开服。',
            'No joinable worlds found. Make sure others have clicked "Open to LAN" in-game or started a server.'
          )}
        />
      ) : (
            ...
            >
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                <div style={{ marginTop: 4, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  ...
                  <Tag color="green">
                    {s.onlinePlayers}/{s.maxPlayers} {tl('人', '')}
                  </Tag>
