...
const HOTKEY_ITEMS: Array<{
...
}> = [
  {
    ...
    zhDesc: '一键开启或关闭自己的麦克风',
    enDesc: 'Turn your microphone on or off',
  },
  {
    key: 'globalMuteHotkey',
    zh: '全局听筒开关',
    en: 'Toggle Speaker',
    zhDesc: '一键静音或恢复所有人的语音',
    enDesc: 'Mute or unmute everyone at once',
  },
  {
    key: 'pushToTalkHotkey',
    zh: '临时开麦',
    en: 'Push to Talk',
    zhDesc: '按住时开麦，松开后自动恢复原状态',
    enDesc: 'Mic opens while held, restores when released',
  },
  {
    key: 'summonHotkey',
    zh: '唤出主窗口',
    en: 'Summon Main Window',
    zhDesc: '从系统托盘或最小化状态把窗口唤回最前',
    enDesc: 'Bring the window back to front from tray or minimized state',
  },
];
...
export const SettingsWindow: React.FC<{ onClose: () => void }> = ({ onClose }) => {
      ...
      try {
        await invoke('save_settings', {
          ...
          rememberWindowPosition:
            merged.rememberWindowPosition !== undefined ? merged.rememberWindowPosition : false,
          ...
          enableGpuRendering:
            merged.enableGpuRendering !== undefined ? merged.enableGpuRendering : true,
  ...
  const handleHotkeyChange = useCallback(
    async (key: HotkeyKey, value: string) => {
      const normalized = value.trim();
      // 冲突校验：不允许与其它已配置的快捷键重复（留空表示禁用，不参与校验）
      if (normalized) {
        const conflict = HOTKEY_ITEMS.find(
          (it) =>
            it.key !== key && (hotkeys[it.key] || '').toLowerCase() === normalized.toLowerCase()
        );
        if (conflict) {
          message.error(
            tl(
              `快捷键 ${normalized} 已被「${conflict.zh}」占用，请换一个`,
      ...
      try {
        ...
        message.error(
          tl(
            '快捷键已保存，但注册失败（可能与其它软件冲突）',
            'Hotkey saved, but registration failed (it may conflict with another app)'
          )
        );
      }
    },
  ...
  return (
    <div className="settings-window">
      ...
      <div className="settings-window-scroll">
        <motion.div
          className="settings-window-inner"
          ...
          >
              ...
              <div className="settings-card-header">
                <div className="settings-card-icon settings-card-icon-purple">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 3a9 9 0 1 0 9 9c0-.46-.04-.92-.1-1.36a5.39 5.39 0 0 1-4.4 2.26 5.4 5.4 0 0 1-5.4-5.4 5.39 5.39 0 0 1 2.26-4.4A9.03 9.03 0 0 0 12 3zm2.46 2.03a3.4 3.4 0 0 0 4.51 4.51A7.02 7.02 0 0 1 14.46 5.03z" />
                  </svg>
                </div>
                <span className="settings-card-title">{t('settings.theme')}</span>
              </div>
              <div className="settings-card-desc">{t('settings.themeDesc')}</div>
              <div className="settings-centered-control settings-theme-control">
                <Segmented
                  value={themeMode}
                  onChange={(v) => {
                      ...
                      label: (
                        <span className="settings-theme-option">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M12 7a5 5 0 1 0 0 10 5 5 0 0 0 0-10zm0-3a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm8 7a1 1 0 1 0 0 2 1 1 0 0 0 0-2zM4 11a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm9-8a1 1 0 1 0-2 0 1 1 0 0 0 2 0zm0 18a1 1 0 1 0-2 0 1 1 0 0 0 2 0zm9-4a1 1 0 1 0 0-2 1 1 0 0 0 0 2zM5.34 6.34a1 1 0 1 0 1.41-1.41 1 1 0 0 0-1.41 1.41zm12.73 12.73a1 1 0 1 0 1.41-1.41 1 1 0 0 0-1.41 1.41zM5.34 17.66a1 1 0 1 0 1.41 1.41 1 1 0 0 0-1.41-1.41zM18.07 6.34a1 1 0 1 0-1.41-1.41 1 1 0 0 0 1.41 1.41z" />
                          </svg>
                          {tl('明亮', 'Light')}
                        </span>
                      ),
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              ...
              <div className="settings-toggle-row">
                <div className="settings-toggle-info">
                  <span className="settings-toggle-label">
                    {tl('开机自启动', 'Launch on Startup')}
                  </span>
                  <span className="settings-toggle-desc">
                    {tl(
                      'MCTier 将在系统启动时自动运行',
                      'MCTier will run automatically when the system starts'
                    )}
                  </span>
                </div>
                <Switch
                  checked={autoStartup}
                  onChange={handleAutoStartupChange}
              ...
              <div className="settings-toggle-row">
                <div className="settings-toggle-info">
                  ...
                  <span className="settings-toggle-desc">
                    {tl('保持窗口始终显示在最前面', 'Keep the window always in front')}
                  </span>
                </div>
                <Switch
                  checked={alwaysOnTop}
              ...
              <div className="settings-toggle-row">
                <div className="settings-toggle-info">
                  <span className="settings-toggle-label">
                    {tl('记住窗口位置', 'Remember Window Position')}
                  </span>
                  <span className="settings-toggle-desc">
                    {tl(
                      '启动时恢复上次关闭时的窗口位置',
              ...
              <div className="settings-toggle-row">
                <div className="settings-toggle-info">
                  <span className="settings-toggle-label">
                    {tl('关闭时最小化到托盘', 'Minimize to Tray on Close')}
                  </span>
                  <span className="settings-toggle-desc">
                    {tl(
                      '点击关闭按钮时隐藏到系统托盘后台运行，而不是退出程序',
                      'Hide to the system tray and keep running in the background instead of quitting when you click close'
                    )}
                  </span>
                </div>
                <Switch
              ...
              <div className="settings-toggle-row">
                <div className="settings-toggle-info">
                  <span className="settings-toggle-label">
                    {tl('启动后自动隐藏到托盘', 'Start Minimized to Tray')}
                  </span>
                  <span className="settings-toggle-desc">
                    {tl(
                      'MCTier 启动后自动隐藏到系统托盘，在后台静默运行',
                      'MCTier automatically hides to the system tray after launch and runs silently in the background'
                    )}
                  </span>
                </div>
                <Switch
              ...
              <div className="settings-toggle-row">
                <div className="settings-toggle-info">
                  <span className="settings-toggle-label">
                    {tl('启用 GPU 渲染', 'Enable GPU Rendering')}
                  </span>
                  <span className="settings-toggle-desc">
                    {tl(
                      '关闭可降低 GPU 占用，但会禁用部分动画效果',
                      'Disabling reduces GPU usage but turns off some animations'
                    )}
                  </span>
                </div>
                <Switch
                  checked={enableGpuRendering}
                  onChange={async (v) => {
                    // 先保存设置
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              ...
              <div className="settings-toggle-row">
                <div className="settings-toggle-info">
                  <span className="settings-toggle-label">
                    {tl('启动时自动创建/加入大厅', 'Auto create/join lobby on startup')}
                  </span>
                  <span className="settings-toggle-desc">
                    {tl(
                      'MCTier 启动后自动进入指定大厅',
                      'MCTier automatically enters the specified lobby after launch'
                    )}
                  </span>
                </div>
                <Switch
                  checked={autoLobbyEnabled}
                  onChange={handleAutoLobbyChange}
                  className="settings-switch"
                />
              </div>
              <AnimatePresence>
                    ...
                    <div className="settings-sub-form">
                      <Form.Item
                        ...
                        rules={[
                          {
                            required: true,
                            message: tl('请输入大厅名称', 'Please enter a lobby name'),
                          },
                          {
                            whitespace: true,
                            message: tl('不能为空白字符', 'Cannot be only whitespace'),
                          },
                          {
                            min: 4,
                            max: 32,
                            message: tl('长度为 4-32 个字符', 'Length must be 4-32 characters'),
                          },
                          {
                            pattern: /^[\u4e00-\u9fa5a-zA-Z0-9_\-\s]+$/,
                            message: tl(
                              '只能含中文、字母、数字、下划线、连字符和空格',
                      ...
                      >
                        <Input
                          placeholder={tl('4-32 个字符', '4-32 characters')}
                          maxLength={32}
                      ...
                      <Form.Item
                        name="lobbyPassword"
                        label={tl('大厅密码', 'Lobby Password')}
                        rules={[
                          { required: true, message: tl('请输入密码', 'Please enter a password') },
                          {
                            min: 8,
                            max: 32,
                            message: tl('长度 8-32 个字符', 'Length must be 8-32 characters'),
                          },
                          {
                            validator: (_, v) => {
                              ...
                              if (!/[a-zA-Z]/.test(v))
                                return Promise.reject(
                                  new Error(tl('必须含字母', 'Must contain letters'))
                                );
                              if (!/[0-9]/.test(v))
                                return Promise.reject(
                                  new Error(tl('必须含数字', 'Must contain digits'))
                                );
                      ...
                      <Form.Item
                        ...
                        rules={[
                          {
                            required: true,
                            message: tl('请输入玩家名称', 'Please enter a player name'),
                          },
                          {
                            min: 1,
                            max: 8,
                            message: tl('长度 1-8 个字', 'Length must be 1-8 characters'),
                          },
                        ]}
                      >
                        <Input
                          placeholder={tl('最多 8 个字', 'Up to 8 characters')}
                          maxLength={8}
                          onBlur={handleFieldBlur}
                        />
                      </Form.Item>
                      ...
                      >
                        {tl(
                          ...
                          'Note: Auto lobby uses the same server as manual lobby creation. If "Use private server" is enabled, it uses the EasyTier node and signaling server you configured; otherwise it uses the node from the last successful lobby (default: MCTier official server).'
                        )}
                      </div>
                    </div>
                  </motion.div>
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              <div className="settings-card-header">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flex: 1 }}>
                  ...
                  <Tooltip
                    title={tl(
                      'MCTier 官网提供后端源码与私有化部署教学，您可以自行搭建私有服务器',
                      'The MCTier website provides backend source code and self-hosting tutorials so you can run your own server'
                    )}
                    placement="right"
                  >
              ...
              <div className="settings-toggle-row">
                <div className="settings-toggle-info">
                  <span className="settings-toggle-label">
                    {tl('使用私有服务器', 'Use Private Server')}
                  </span>
                  <span className="settings-toggle-desc">
                    {tl(
                      '启用后可配置自己部署的服务器',
                      'Enable to configure your own deployed server'
                    )}
                  </span>
                </div>
                <Switch
              ...
              <AnimatePresence>
                {usePrivateServer && (
                  ...
                  >
                    <div className="settings-sub-form">
                      <Form.Item
                        name="privateEasytierServer"
                        label={tl('EasyTier 节点服务器', 'EasyTier Node Server')}
                        rules={[
                          {
                            required: true,
                            message: tl(
                          ...
                          },
                          {
                            ...
                            message: tl(
                              ...
                              'Format: must start with tcp://, udp://, ws://, wss:// or txt://'
                            ),
                          },
                        ]}
                      >
                      ...
                      <Form.Item
                        name="privateSignalingServer"
                        label={tl('WebRTC 信令服务器', 'WebRTC Signaling Server')}
                        rules={[
                          {
                            required: true,
                            message: tl(
                      ...
                      >
                        {tl(
                          ...
                          'Note: The MCTier website only provides the signaling server source code'
                        )}
                      </div>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            message.success(
                              tl(
                                '已重置为默认私有服务器地址',
                                'Reset to default private server address'
                              )
                            );
                          }}
                        >
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              <div className="settings-card-header">
                ...
                <span className="settings-card-title">
                  {tl('自定义 EasyTier 节点', 'Custom EasyTier Nodes')}
                </span>
              </div>
              <div className="settings-card-desc">
                {tl(
                  '添加备用节点，创建/加入大厅时可在下拉中选择',
                  'Add backup nodes to pick from when creating/joining a lobby'
                )}
              </div>
              <CustomNodeManager />
            </motion.div>
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              <div className="settings-card-header">
                ...
                <span className="settings-card-title">
                  {tl('全局 EasyTier 高级配置', 'Global EasyTier Advanced Config')}
                </span>
              </div>
              <div className="settings-card-desc">
                {tl(
                  '配置 EasyTier 的高级参数，这些配置将作为默认配置应用于所有大厅',
                  'Configure EasyTier advanced parameters; these apply as defaults to all lobbies'
                )}
              </div>
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              ...
              <div className="settings-card-desc">
                {tl(
                  '点击输入框后按下你想要的组合键即可录制，修改后立即生效，无需重启。留空表示禁用该快捷键。',
                  'Click a field and press your desired key combination to record it. Changes take effect immediately without restarting. Leave empty to disable a hotkey.'
                )}
              </div>
              {HOTKEY_ITEMS.map((item) => (
                <div className="settings-toggle-row settings-toggle-row-hotkey" key={item.key}>
                  <div className="settings-toggle-info">
                  ...
                  <div className="settings-hotkey-control">
                    <HotkeyInput
                      ...
                      onChange={(v) => {
                        void handleHotkeyChange(item.key, v);
                      }}
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              ...
              <div className="settings-card-desc">
                {tl(
                  ...
                  'Customize sounds (restorable to default), adjust volume and set Do Not Disturb hours'
                )}
              </div>
              <SoundThemeManager />
            </motion.div>
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              ...
              <div className="settings-card-desc">
                {tl(
                  '开启后聊天消息会以弹幕形式从屏幕顶部飘过，游戏时也不错过；可调字号、速度、透明度、轨道数并预览',
                  'When enabled, chat messages float across the top of the screen so you never miss them while gaming; adjust size, speed, opacity, tracks and preview'
                )}
              </div>
              <DanmakuSettings />
            </motion.div>
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              ...
              <div className="settings-card-desc">
                {tl(
                  '玩游戏时在屏幕角落显示队友延迟、谁在说话等状态；可在此调整浮层透明度。开关入口在「房间工具 - 联机」',
                  'Show teammate latency and who is speaking in a screen corner while gaming; adjust overlay opacity here. Toggle it in Room Tools - Networking'
                )}
              </div>
              <GameHudSettings />
            </motion.div>
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              ...
              <div className="settings-card-desc">
                {tl(
                  '选择进入大厅后默认使用的音色，开麦即生效；进大厅后也可在大厅动态设置中一键切换',
                  'Choose the default voice used after joining a lobby; you can also switch it live in lobby settings'
                )}
              </div>
              <VoiceChangerPicker />
            </motion.div>
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              ...
              <div className="settings-card-desc">{t('settings.dataStatsDesc')}</div>
              <div className="settings-centered-control">
            ...
            <motion.div className="settings-card" variants={itemVariants}>
              ...
              <div className="settings-card-desc">{t('settings.configMgrDesc')}</div>
              <ConfigManager />
            </motion.div>
          </Form>
        </motion.div>
      ...
      <RestartConfirmModal
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            message.error(
              tl('重启应用失败，请手动重启', 'Failed to restart the app, please restart manually')
            );
          }
        }}
        onCancel={() => {
          setShowRestartModal(false);
          message.info(
            tl('设置已保存，下次启动时生效', 'Settings saved, effective on next launch')
          );
        }}
      />
    </div>
...
const SoundThemeManager: React.FC = () => {
  ...
  const previewSound = async (target: SoundType) => {
    try {
      ...
      antdMessage.error(
        tl(
          '试听失败，请检查默认音效文件或自定义音频文件是否可用',
          'Preview failed. Please check the sound file.'
        )
      );
    }
  };
  ...
  const onPickFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      antdMessage.success(
        `${tl('已设置', 'Set custom sound for')}「${labels[target]}」${tl('自定义提示音', '')}`
      );
    };
      ...
      <div className="snd-block">
        ...
        <Slider
          ...
          onChange={(v) => {
            ...
            audioService.setVolume(v as number);
          }}
        ...
        {(Object.keys(labels) as SoundType[]).map((t) => (
          <div className="snd-card" key={t}>
            <div className="snd-card-left">
              ...
              <span className={`snd-card-tag ${custom[t] ? 'is-custom' : ''}`}>
                {custom[t] ? tl('自定义', 'Custom') : tl('默认音', 'Default')}
              </span>
            </div>
            <div className="snd-card-actions">
              ...
              {custom[t] && (
                <button
                  className="snd-text-btn snd-reset"
                  title={tl('恢复为默认提示音', 'Restore default sound')}
                  onClick={() => {
                    audioService.resetSound(t);
                    setCustom({ ...audioService.getSettings().custom });
                    antdMessage.success(tl('已恢复默认提示音', 'Default sound restored'));
                  }}
                >
                  {tl('恢复默认', 'Default')}
                </button>
              ...
              <Switch
                ...
                onChange={(v) => {
                  setMutedSounds((prev) => ({ ...prev, [t]: !v }));
                  audioService.setSoundMuted(t, !v);
                }}
              />
            </div>
          </div>
      ...
      <div className="snd-block">
        <div className="snd-dnd-head">
          <div>
            ...
            <div className="snd-block-desc">
              {tl(
                '开启后，下方时段内不播放任何提示音',
                'When enabled, no sounds play during the hours below'
              )}
            </div>
          </div>
          <Switch
        ...
        {dndEnabled && (
          <div className="snd-dnd-times">
            <input
              type="time"
              value={minutesToHHMM(dndStart)}
              onChange={(e) => {
                const [h, m] = e.target.value.split(':').map(Number);
                const mins = h * 60 + m;
                setDndStart(mins);
                audioService.setDnd(true, mins, dndEnd);
              }}
            />
            <span className="snd-dnd-sep">{tl('至', 'to')}</span>
            <input
              type="time"
              value={minutesToHHMM(dndEnd)}
              onChange={(e) => {
                const [h, m] = e.target.value.split(':').map(Number);
                const mins = h * 60 + m;
                setDndEnd(mins);
                audioService.setDnd(true, dndStart, mins);
              }}
            />
          </div>
        )}
...
const CustomNodeManager: React.FC = () => {
  ...
  const saveNodes = async (newNodes: EasyTierNode[]) => {
    try {
      // 过滤掉默认节点，只保存用户自定义的节点
      const customNodesOnly = newNodes.filter(
        (node) => !BUILTIN_NODE_ADDRESSES.includes(node.address)
      );
  ...
  const handleSave = async () => {
    ...
    if (!addressPattern.test(editForm.address.trim())) {
      message.error(
        tl(
          '节点地址格式错误，应以 tcp://、udp://、ws://、wss:// 或 txt:// 开头',
          'Invalid node address format; it must start with tcp://, udp://, ws://, wss:// or txt://'
        )
      );
      return;
    }
    ...
    if (duplicated) {
      message.warning(
        tl(
          '该节点地址已存在，请勿重复添加',
          'This node address already exists, do not add it again'
        )
      );
      return;
    }
  ...
  const handleDelete = async (index: number) => {
    ...
    modal.confirm({
      ...
      content: tl(
        `确定要删除节点「${target?.name ?? ''}」吗？此操作不可恢复。`,
        `Delete node "${target?.name ?? ''}"? This cannot be undone.`
      ),
      okText: tl('删除', 'Delete'),
      okType: 'danger',
  ...
  if (loading) {
    return (
      <div style={{ padding: '16px', textAlign: 'center', color: 'var(--mc-text-3)' }}>
        {tl('加载中...', 'Loading...')}
      </div>
    );
  }
          ...
          >
            {editingIndex === index ? (
              ...
              <div className="node-edit-form">
                ...
                <Input
                  placeholder={tl(
                    '节点地址 (例如: wss://example.com)',
                    'Node address (e.g. wss://example.com)'
                  )}
  ...
  const handleImport = async () => {
    try {
      ...
      try {
        ...
        message.success(
          tl('配置导入成功，设置已更新', 'Config imported successfully, settings updated')
        );
      } catch (error) {
    ...
    <div className="config-manager">
      <div className="config-manager-buttons">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          title={tl(
            '将运行日志打包为 zip 导出到桌面，方便反馈问题',
            'Package runtime logs as a zip and export to the desktop for easy reporting'
          )}
        >
          {exportingLogs ? (
            <>
