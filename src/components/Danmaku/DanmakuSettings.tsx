    ...
    <div className="snd-manager">
      ...
      >
        <div>
          ...
          <div className="snd-block-desc">
            {tl(
              '聊天消息将以弹幕飘过屏幕顶部，并置顶于其他窗口之上',
              'Chat messages float across the top, above other windows'
            )}
          </div>
        </div>
      ...
      <div className="snd-block">
        <div className="snd-block-title">
          <span>{tl('字号', 'Font Size')}</span>
          <span className="snd-vol-val">{cfg.fontSize}px</span>
        </div>
        <Slider
          min={14}
          max={48}
      ...
      <div className="snd-block">
        <div className="snd-block-title">
          <span>{tl('滚动速度', 'Speed')}</span>
          <span className="snd-vol-val">{cfg.speed}px/s</span>
        </div>
        <Slider
          min={60}
          max={320}
          step={10}
          value={cfg.speed}
          onChange={(v) => update({ speed: v as number })}
        />
      </div>
      <div className="snd-block">
        <div className="snd-block-title">
          <span>{tl('不透明度', 'Opacity')}</span>
          <span className="snd-vol-val">{Math.round(cfg.opacity * 100)}%</span>
        </div>
        <Slider
          min={0.2}
          max={1}
      ...
      <div className="snd-block">
        <div className="snd-block-title">
          <span>{tl('弹幕轨道数', 'Tracks')}</span>
          <span className="snd-vol-val">{cfg.tracks}</span>
        </div>
        <Slider
          min={1}
          max={10}
          ...
          {['#ffffff', '#07c160', '#1890ff', '#fa9d3b', '#fa5151', '#eb2f96'].map((c) => (
            <span
              ...
              style={{
                ...
                flexShrink: 0,
                border:
                  cfg.color.toLowerCase() === c
                    ? '2px solid rgba(0,0,0,0.5)'
                    : '2px solid rgba(0,0,0,0.15)',
                boxShadow: cfg.color.toLowerCase() === c ? '0 0 6px rgba(0,0,0,0.3)' : 'none',
              }}
            />
          ))}
          <span
            ...
            style={{
              ...
              cursor: 'pointer',
              flexShrink: 0,
              background: 'conic-gradient(#fa5151,#fa9d3b,#07c160,#1890ff,#eb2f96,#fa5151)',
              border:
                cfg.color === 'rainbow'
                  ? '2px solid rgba(0,0,0,0.5)'
                  : '2px solid rgba(0,0,0,0.15)',
              boxShadow: cfg.color === 'rainbow' ? '0 0 6px rgba(0,0,0,0.3)' : 'none',
            }}
          />
          <input
            type="color"
      ...
      <div className="snd-block">
        ...
        <button
          ...
          style={{ marginTop: 8 }}
          onClick={() => {
            void danmakuService.preview(tl('这是一条弹幕预览', 'This is a danmaku preview'));
            antdMessage.success(tl('已在屏幕上预览', 'Previewing on screen'));
          }}
        >
